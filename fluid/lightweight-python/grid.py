...
class Point:
    ...
    def update(self, surrounding):
        for i in range(4):
            self.flow[i] = max(self.value - surrounding[i], 0)
        if self.value > 1:
            print(self.value)
...
while running:
    ...
    for y in range(N):
        for x in range(N):
            if y == 0:
                points[y+1][x].value += points[y][x].flow[0] * damping
                points[y][x].value -= points[y][x].flow[0] * damping
            elif y == N-1:
                points[y-1][x].value += points[y][x].flow[2] * damping
                points[y][x].value -= points[y][x].flow[2] * damping
            else:
                points[y+1][x].value += points[y][x].flow[0] * damping
                points[y-1][x].value += points[y][x].flow[2] * damping
                points[y][x].value -= points[y][x].flow[0] * damping
            ...
            if x == 0:
                points[y][x+1].value += points[y][x].flow[1] * damping
                points[y][x].value -= points[y][x].flow[1] * damping
            elif x == N-1:
                points[y][x-1].value += points[y][x].flow[3] * damping
                points[y][x].value -= points[y][x].flow[3] * damping
            else:
                points[y][x+1].value += points[y][x].flow[1] * damping
                points[y][x-1].value += points[y][x].flow[3] * damping
                points[y][x].value -= points[y][x].flow[1] * damping
